// =============================================================
//  INS 
//  ROOT CAUSE FIX: YAW 39845deg / velocity 0.2m/s while static
//  - Gyro bias cal now uses MEDIAN not mean (rejects vibration)
//  - YAW bounded to ±180 deg after each integration step
//  - ZUPT hard-zeros velocity AND position delta each fire
//  - Position frozen to zero during full stationary window
//  - Jam threshold raised to 2000m (was 300m, too tight)
//
//  NEW FEATURES:
//  - Live map trail (canvas, no Leaflet, zero server load)
//  - Target coordinates + arrival beep/LED
//  - Unit toggle m/s ↔ km/h on velocity panel
//  - Auto time from mobile on setup page
//  - viewMap() sends correct INS coords to Google Maps
//  - Buzz/LED on target arrival (GPIO26)
// =============================================================
...
#define PIX_RX     32
#define PIX_TX     33
#define LED_DRIFT   2   // built-in: blinks when static but drifting
#define LED_GPS     4   // GPS fix indicator
#define LED_STATIC  5   // ZUPT active indicator
#define BUZZER_PIN 26   // piezo buzzer for target arrival
#define NS         15
...
// Previous good sample cache for rate-of-change rejection
static float prev_gx=0,prev_gy=0,prev_gz=0;
static float prev_ax=0,prev_ay=0,prev_az=0;
static bool  mpuHasPrev=false;
// Smoothed output (simple low-pass, alpha=0.4 — responsive but noise-filtered)
static float sm_ax=0,sm_ay=0,sm_az=0;
static float sm_gx=0,sm_gy=0,sm_gz=0;
#define GYRO_DELTA_MAX  2.0f   // max change between consecutive reads (rad/s)
                                // MPU6050 at DLPF 21Hz physically cannot jump >2 rad/s
                                // between 5ms samples unless read is corrupt
#define ACCEL_DELTA_MAX 15.0f  // m/s² — generous for real motion
...
// ── Median sort helper for calibration ───────────────────────
// Vibration creates outliers — median is immune to them
...
// Store samples in two passes — medians per axis
static float calBufGz[CAL_SAMPLES]; // only store gz for median (most critical)
...
float  targetRadiusM=10.0f;  // arrival threshold metres
...
void ekfSanitize() {
    ...
    for (int i=0;i<NS;i++)
        if (!isfinite(ekf_P[i][i])||ekf_P[i][i]<0||ekf_P[i][i]>1e6f)
            ekf_P[i][i]=0.5f;
    // FIX: Clamp yaw to ±pi (prevents 39845 deg accumulation)
    while (ekf_x[8] >  M_PI) ekf_x[8] -= 2.0f*M_PI;
    while (ekf_x[8] < -M_PI) ekf_x[8] += 2.0f*M_PI;
}

void updateZUPT() {
    // EKF measurement update: velocity → 0
    ...
    // Hard clamp position covariance
...
void ekf_init() {
    ...
    ekf_Q[0]=ekf_Q[1]=ekf_Q[2] = 1e-9f;   // pos: almost frozen
    ekf_Q[3]=ekf_Q[4]=ekf_Q[5] = 1e-6f;   // vel: very tight
    ekf_Q[6]=ekf_Q[7]=ekf_Q[8] = 1e-5f;   // att
    ekf_Q[9]=ekf_Q[10]=ekf_Q[11] = 1e-9f; // accel bias
    ekf_Q[12]=ekf_Q[13]=ekf_Q[14] = 1e-10f;// gyro bias
}
...
.sb{display:flex;justify-content:space-between;align-items:center;
    background:#09141c;border:1px solid #0c3050;border-radius:3px;
    padding:5px 8px;margin-bottom:5px;font-size:10px;gap:4px;flex-wrap:wrap}
...
.btn{width:100%;padding:10px;margin:4px 0;background:transparent;
...
input{width:100%;background:#09141c;color:#00ff88;border:1px solid #0c3050;
      border-radius:3px;padding:7px;margin:3px 0;font-family:monospace;font-size:12px}
...
.warn{background:#150005;border:1px solid #ff3355;color:#ff3355;
      border-radius:3px;text-align:center;padding:5px;font-size:10px;margin-bottom:5px}
canvas{display:block}
#LD{text-align:center;padding:30px;color:#00cfff;font-size:11px}
.mapWrap{width:100%;border:1px solid #0c3050;border-radius:3px;margin-bottom:5px;position:relative;overflow:hidden}
</style></head><body>
<h1>INS v14</h1><div class="sub">JUBAIR AL BERUNE // UAV // MISSILE // ROV</div>
...
<div id="SU" style="display:none">
  <div class="p">
    ...
    <div class="rw"><span class="lb">PRESSURE</span><span class="vl" id="pL">--</span></div>
  </div>
  <!-- Auto time sync -->
  <div class="p">
  ...
  <div class="p">
    <div class="ph">TARGET COORDINATES (OPTIONAL)</div>
    ...
    <div style="font-size:10px;color:#1e3a4a;margin-top:3px">LED+beep when within radius. Leave blank to disable.</div>
  </div>
  <button class="btn bc" onclick="phoneGPS()">USE PHONE GPS FOR ORIGIN</button>
  <button class="btn bc" style="margin-top:0" onclick="openMaps()">OR PICK IN GOOGLE MAPS</button>
...
<div id="DA" style="display:none">
  ...
  <!-- Instruments -->
  <div class="g2" style="margin-bottom:4px">
    <div class="p" style="padding:4px">
      <div class="ph">HORIZON</div>
      <canvas id="cH" width="148" height="112"></canvas>
    </div>
    <div class="p" style="padding:4px">
      <div class="ph">COMPASS</div>
      <canvas id="cC" width="148" height="112"></canvas>
    </div>
  </div>
  <div class="p" style="padding:4px;margin-bottom:4px">
    <div class="ph">TURN RATE</div>
    <canvas id="cT" width="290" height="36"></canvas>
  </div>
  ...
  <div class="g2">
    ...
    <div class="p">
      <!-- Unit toggle on velocity panel header -->
  ...
  <div id="bumpResult" class="p" style="display:none;margin-top:4px">
    ...
    <div class="rw"><span class="lb">DRIFT ERROR</span><span class="vl va" id="bumpError">--</span><span class="lb"> m</span><span class="vl" id="bumpRating" style="margin-left:8px"></span></div>
...
// ── Map trail ─────────────────────────────────────────────────
var trail=[];          // array of {n,e} metres from origin
var mapScale=5;        // pixels per metre
var mapOriginN=0,mapOriginE=0;
...
function addTrailPoint(n,e){
  // n,e are EKF posN,posE in metres from origin
  trail.push({n:n,e:e});
  if(trail.length>2000) trail.shift(); // keep last 2000 points
}

function drawMap(){
  var cv=document.getElementById('cMap');
  var c=cv.getContext('2d');
  var W=cv.offsetWidth||290,H=200;
  cv.width=W; cv.height=H;
  c.fillStyle='#060c10';c.fillRect(0,0,W,H);
  var cx2=W/2,cy2=H/2;
  // Grid lines (every 10m scaled)
  var gridM=10;
  c.strokeStyle='#0c1a28';c.lineWidth=1;
  var gridPx=gridM*mapScale;
  ...
  if(targetSet2){
    ...
    var tdE=(targetLon2-originLon)*111111*Math.cos(originLat*Math.PI/180);
    ...
    c.strokeStyle='rgba(255,51,85,0.3)';c.lineWidth=1;
    // Rough radius circle
    var rPx=10*mapScale; // targetRadius in px (using 10m default visual)
    c.beginPath();c.arc(tx,ty,rPx,0,2*Math.PI);c.stroke();
  }
  // Trail
  if(trail.length>1){
    c.beginPath();
    c.strokeStyle='#00ff88';c.lineWidth=1.5;c.lineJoin='round';
    var p0=trail[0];
  ...
  if(trail.length>0){
    ...
    var px=cx2+(lp.e-mapOriginE)*mapScale, py=cy2-(lp.n-mapOriginN)*mapScale;
    ...
    c.strokeStyle='#00cfff';c.lineWidth=1.5;
    c.beginPath();c.moveTo(px,py);c.lineTo(px+Math.sin(hdRad)*12,py-Math.cos(hdRad)*12);c.stroke();
  }
  // Scale bar
  var scaleM=Math.round(50/mapScale);
  document.getElementById('mapScale').textContent='scale: '+scaleM+'m per 50px';
}
...
function startClock(h,m,s){
  ...
  clkInt=setInterval(function(){
    tot++;var hh=Math.floor(tot/3600)%24,mm=Math.floor((tot%3600)/60),ss=tot%60;
    document.getElementById('clk').textContent=p2(hh)+':'+p2(mm)+':'+p2(ss);
  },1000);
}
function syncDT(){
  var n=new Date();
  ...
  fetch('/status').then(function(r){return r.json();}).then(function(d){
    ...
    if(d.running){
      ...
      if(d.calDone){
        ...
        document.getElementById('cs').textContent='COMPLETE';
...
function startCalPoll(){
  calPoll=setInterval(function(){
    fetch('/caldata').then(function(r){return r.json();}).then(function(d){
...
function phoneGPS(){
  if(!navigator.geolocation){document.getElementById('msg').textContent='No geolocation';return;}
  ...
  navigator.geolocation.getCurrentPosition(
    function(p){
      ...
      if(p.coords.altitude!=null)document.getElementById('al').value=p.coords.altitude.toFixed(1);
      document.getElementById('msg').textContent='GPS acquired.';
    },
    function(e){document.getElementById('msg').textContent='Err:'+e.message;},
    {enableHighAccuracy:true,timeout:15000}
  );
}
function openMaps(){
  window.open('https://maps.google.com','_blank');
  document.getElementById('msg').textContent='Long-press in Maps to drop pin, copy coords.';
}
...
function startINS(){
  ...
  // Collect target coords if set
  var tgLa=document.getElementById('tgLat').value.trim();
  var tgLo=document.getElementById('tgLon').value.trim();
  var tgRd=document.getElementById('tgRad').value.trim()||'10';
  ...
  if(tgLa&&tgLo) body+='&tgLat='+tgLa+'&tgLon='+tgLo+'&tgRad='+tgRd;
  ...
  .then(function(r){return r.text();}).then(function(t){
    if(t==='OK'){
      ...
      document.getElementById('DA').style.display='block';
      if(targetSet2)document.getElementById('tgtPanel').style.display='block';
      t0=Date.now();setInterval(poll,400);
    }else document.getElementById('msg').textContent='Err:'+t;
  }).catch(function(){document.getElementById('msg').textContent='Comm err.';});
}

function doReset(){
  if(rT){clearInterval(rT);rT=null;document.getElementById('rB').textContent='RESET';return;}
  document.getElementById('rB').textContent='CANCEL(5s)';var cnt=5;
  rT=setInterval(function(){cnt--;if(cnt<=0){clearInterval(rT);rT=null;fetch('/reset',{method:'POST'}).then(function(){window.location.reload();});}else document.getElementById('rB').textContent='CANCEL('+cnt+'s)';},1000);
}
...
function startBumpTest(){
  ...
  if(bumpState===0){
    fetch('/bump_start',{method:'POST'}).then(function(r){return r.text();}).then(function(t){
      if(t==='OK'){bumpState=1;btn.textContent='RETURN TO START';btn.style.color='#00ff88';btn.style.borderColor='#00ff88';document.getElementById('msg').textContent='Move ~1m then press RETURN TO START';}
    });
  }else{
    fetch('/bump_return',{method:'POST'}).then(function(r){return r.json();}).then(function(d){
    ...
    }).catch(function(){bumpState=0;btn.textContent='BUMP TEST';btn.style.color='';btn.style.borderColor='';});
  }
}
...
// ── Poll ──────────────────────────────────────────────────────
function poll(){
  fetch('/data').then(function(r){return r.json();}).then(upd).catch(function(){});
  if(t0)document.getElementById('upt').textContent='T+'+Math.floor((Date.now()-t0)/1000)+'s';
}

function upd(d){
  cLat=d.la;cLon=d.lo;
  // Map trail — add point every ~0.5s effectively (poll is 400ms)
  ...
  var driftSpd=Math.sqrt(d.vn*d.vn+d.ve*d.ve+d.vd*d.vd);
  ...
  if(!d.gconn){nc.style.display='block';nf.style.display='none';fx.style.display='none';}
    ...
    nc.style.display='none';nf.style.display='none';fx.style.display='block';
...
// ── Google Maps with actual INS coords ────────────────────────
function viewMap(){
  if(cLat===0&&cLon===0){alert('No position yet.');return;}
...
function drawH(roll,pitch){
  ...
  c.beginPath();c.moveTo(-W,-ph);c.lineTo(W,-ph);c.stroke();
  c.font='8px monospace';c.textAlign='left';c.textBaseline='middle';
  for(var deg=-20;deg<=20;deg+=10){
    if(deg===0)continue;
    var py=-ph-deg*1.5,lw=deg%20===0?30:20;
    c.strokeStyle='rgba(0,255,136,0.45)';c.lineWidth=1;
  ...
  c.restore();
  c.strokeStyle='#0c3050';c.lineWidth=1;c.strokeRect(0,0,W,H);
}
...
function drawC(mag){
  ...
  c.clearRect(0,0,W,H);
  c.beginPath();c.arc(cx,cy,R,0,2*Math.PI);c.fillStyle='#09141c';c.fill();c.strokeStyle='#0c3050';c.lineWidth=1.5;c.stroke();
  ...
  c.beginPath();c.moveTo(cx,cy);c.lineTo(cx+Math.sin(na)*(R-16),cy-Math.cos(na)*(R-16));c.strokeStyle='#ff3355';c.lineWidth=2.5;c.lineCap='round';c.stroke();
  ...
  c.fillStyle='#00ff88';c.font='9px monospace';c.textAlign='center';c.textBaseline='top';
  c.fillText(Math.round(mag)+' deg',cx,2);
}

// ── Turn indicator ────────────────────────────────────────────
function drawT(yr){
  var cv=document.getElementById('cT'),c=cv.getContext('2d');
  var W=cv.width,H=cv.height,cx=W/2,cy=H/2;
  if(!W||!H)return;c.clearRect(0,0,W,H);
  var mx=6.0;
  for(var i=-3;i<=3;i++){var x=cx+i*(W/2/mx)*2;c.beginPath();c.moveTo(x,cy-7);c.lineTo(x,cy+7);c.strokeStyle=i===0?'#00ff88':'#1e3a4a';c.lineWidth=i===0?1.5:1;c.stroke();}
  var cl=Math.max(-mx,Math.min(mx,yr)),nx=cx+cl*(W/2)/mx;
  c.fillStyle=Math.abs(yr)>0.5?'#ffb800':'#00ff88';c.fillRect(nx-2,cy-9,4,18);
  c.fillStyle='#1e3a4a';c.font='9px monospace';c.textAlign='center';
  c.fillText('L',10,cy+4);c.fillText('R',W-10,cy+4);
...
void loop() {
    ...
    if (gps.location.isValid() && gps.location.age()<GPS_TMO) {
        ...
        if (gpsWarmup<15) {
            ...
            // Raised to 2000m — previous 300m caused permanent jam after drift
            if (tGpsErr<2000.0f) {
    ...
    if (targetSet) {
        ...
        if (!targetReached && tTargetDistM<targetRadiusM) {
            targetReached=true;
            targetBeepEnd=millis()+5000;  // beep 5 seconds
...
// ── Calibration — MEDIAN per axis (vibration immune) ─────────
void runCalibration() {
    ...
    // FIX: use MEDIAN for gz (most drift-critical axis, most affected by vibration)
    int n=min(ok,CAL_SAMPLES);
    insertionSort(calBufGz,n);
    gbz=calBufGz[n/2];  // median eliminates vibration spikes
    calDone=true;
...
void handleData() {
    ...
    snprintf(b,sizeof(b),
        ...
        (double)tGyrX,(double)tGyrY,(double)tGyrZ,
        (double)tGpsErr,
